...
function ADD_SREDNJA_SKOLA(payload) {
    return `INSERT INTO srednjaSkola (naziv, gradID, drzavaID, tip) SELECT '${payload.naziv}', (SELECT gradID FROM grad WHERE naziv = '${payload.grad}' LIMIT 1), (SELECT drzavaID FROM drzava WHERE naziv = '${payload.drzava}' LIMIT 1), '${payload.tip}' WHERE (SELECT srednjaSkolaID FROM srednjaSkola WHERE naziv = '${payload.naziv}' AND tip = '${payload.tip}') IS NULL;`;
}
...
function ADD_UNIVERZITET(payload) {
    return `INSERT INTO univerzitet (drzavaID, gradID, naziv) SELECT (SELECT drzavaID FROM drzava WHERE naziv = '${payload.drzava}' LIMIT 1), (SELECT gradID FROM grad WHERE naziv = '${payload.grad}' LIMIT 1), '${payload.univerzitet}' WHERE (SELECT univerzitetID FROM univerzitet WHERE naziv = '${payload.univerzitet}' AND gradID = (SELECT gradID from grad where naziv = '${payload.grad}' LIMIT 1) AND drzavaID = (SELECT drzavaID FROM drzava WHERE naziv = '${payload.drzava}' LIMIT 1)) IS NULL;`;
}
...
function UPDATE_SREDNJA_SKOLA(payload) {
    return `UPDATE ideUSrednju SET srednjaID = (SELECT srednjaSkolaID FROM srednjaSkola WHERE naziv = '${payload.naziv}' AND tip = '${payload.tip}' LIMIT 1), godinaZavrsetka = ${payload.godinaZavrsetka}, smer = '${payload.smer}' WHERE ID = ${payload.id}`;
}

function UPDATE_FAKULTET(payload) {
    return `UPDATE studira SET smerID = (SELECT smerID FROM fakultetSmer WHERE naziv = '${payload.smer}' LIMIT 1), fakultetID = (SELECT fakultetID FROM fakultet WHERE naziv = '${payload.fakultet}' AND univerzitetID = (SELECT univerzitetID FROM univerzitet WHERE naziv = '${payload.univerzitet}' AND drzavaID = (SELECT drzavaID FROM drzava WHERE naziv = '${payload.drzava}' LIMIT 1) AND gradID = (SELECT gradID FROM grad WHERE naziv = '${payload.grad}' LIMIT 1)LIMIT 1) LIMIT 1), godinaUpisa = ${payload.godinaUpisa}, prosek = ${payload.prosek}, status = '${payload.status}', godineStudija = ${payload.godineStudija}, brojPolozenihIspita = ${payload.brojPolozenihIspita}, espb = ${payload.espb} WHERE ID = ${payload.id};`;
}

function DELETE_SREDNJA_SKOLA(payload) {
    return `DELETE FROM ideUSrednju WHERE ID = ${payload};`;
}

function DELETE_FAKULTET(payload) {
    return `DELETE FROM studira WHERE ID = ${payload};`;
}
...
function ADD_RADNO_ISKUSTVO(payload) {
    return `INSERT INTO radnoIskustvo (userID, organizacijaID, datumPocetka, datumZavrsetka, pozicija, opisPosla) SELECT ${payload.userID}, (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.kompanija}' LIMIT 1), '${payload.datumPocetka}', '${payload.datumZavrsetka}', '${payload.funkcija}', '${payload.opis}';`;
}

function GET_RADNO_ISKUSTVO_ID(payload) {
    return `SELECT radnoIskustvoID FROM radnoIskustvo WHERE userID = ${payload.userID} AND organizacijaID = (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.kompanija}' LIMIT 1) AND datumPocetka = '${payload.datumPocetka}' AND datumZavrsetka = '${payload.datumZavrsetka}' AND pozicija = '${payload.funkcija}'`;
}

function DELETE_RADNO_ISKUSTVO(payload) {
    return `DELETE FROM radnoIskustvo WHERE radnoIskustvoID = ${payload}`;
}

function EDIT_RADNO_ISKUSTVO(payload) {
    return `UPDATE radnoIskustvo SET organizacijaID = (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.kompanija}' LIMIT 1), datumPocetka = '${payload.datumPocetka}', datumZavrsetka = '${payload.datumZavrsetka}', pozicija = '${payload.funkcija}', opisPosla = '${payload.opis}' WHERE radnoIskustvoID = ${payload.id}`;
}

function ADD_RAD_NA_PROJEKTU(payload) {
    return `INSERT INTO radNaProjektu (naziv, uloga, datumPocetka, datumZavrsetka, opis, userID) SELECT '${payload.naziv}', '${payload.uloga}', '${payload.datumPocetka}', '${payload.datumZavrsetka}', '${payload.opis}', ${payload.userID};`;
}

function GET_RAD_NA_PROJEKTU_ID(payload) {
    return `SELECT projekatID FROM radNaProjektu WHERE naziv = '${payload.naziv}' AND uloga = '${payload.uloga}' AND datumPocetka = '${payload.datumPocetka}' AND datumZavrsetka = '${payload.datumZavrsetka}' AND userID = ${payload.userID};`;
}

function EDIT_RAD_NA_PROJEKTU(payload) {
    return `UPDATE radNaProjektu SET naziv = '${payload.naziv}', uloga = '${payload.uloga}', datumPocetka = '${payload.datumPocetka}', datumZavrsetka = '${payload.datumZavrsetka}', opis = '${payload.opis}' WHERE projekatID = ${payload.id};`;
}

function DELETE_RAD_NA_PROJEKTU(payload) {
    return `DELETE FROM radNaProjektu WHERE projekatID = ${payload}`;
}

function ADD_STRUCNO_USAVRSAVANJE(payload) {
    return `INSERT INTO strucnoUsavrsavanje (naziv, organizacijaID, datumPocetka, datumZavrsetka, opis, sertifikat, userID) SELECT '${payload.naziv}', (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.organizator}' LIMIT 1), '${payload.datumPocetka}', '${payload.datumZavrsetka}', '${payload.opis}', ${payload.sertifikat}, ${payload.userID};`;
}

function EDIT_STRUCNO_USAVRSAVANJE(payload) {
    return `UPDATE strucnoUsavrsavanje SET organizacijaID = (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.organizator}' LIMIT 1), datumPocetka = '${payload.datumPocetka}', datumZavrsetka = '${payload.datumZavrsetka}', naziv = '${payload.naziv}', opis = '${payload.opis}', sertifikat = ${payload.sertifikat} WHERE strucnoUsavrsavanjeID = ${payload.id}`;
}

function DELETE_STRUCNO_USAVRSAVANJE(payload) {
    return `DELETE FROM strucnoUsavrsavanje WHERE strucnoUsavrsavanjeID = ${payload}`;
}

function GET_STRUCNO_USAVRSAVANJE_ID(payload) {
    return `SELECT strucnoUsavrsavanjeID FROM strucnoUsavrsavanje WHERE userID = ${payload.userID} AND organizacijaID = (SELECT organizacijaID FROM organizacija WHERE imeOrganizacije = '${payload.organizator}' LIMIT 1) AND datumPocetka = '${payload.datumPocetka}' AND datumZavrsetka = '${payload.datumZavrsetka}'`;
}

function ADD_RAD_NA_RACUNARU(payload) {
    return `INSERT INTO radNaRacunaru (naziv, nivo, sertifikat, userID) SELECT '${payload.naziv}', '${payload.nivo}', '${payload.sertifikat}', ${payload.userID};`;
}

function GET_RAD_NA_RACUNARU_ID(payload) {
    return `SELECT radNaRacunaruID FROM radNaRacunaru WHERE naziv = '${payload.naziv}' AND nivo = '${payload.nivo}' AND sertifikat = '${payload.sertifikat}' AND userID = ${payload.userID};`;
}

function EDIT_RAD_NA_RACUNARU(payload) {
    return `UPDATE radNaRacunaru SET naziv = '${payload.naziv}', nivo = '${payload.nivo}', sertifikat = '${payload.sertifikat}' WHERE radNaRacunaruID = ${payload.id};`;
}

function DELETE_RAD_NA_RACUNARU(payload) {
    return `DELETE FROM radNaRacunaru WHERE radNaRacunaruID = ${payload}`;
}

function ADD_JEZIK(payload) {
    return `INSERT INTO jezik (imeJezika) SELECT '${payload.jezik}' WHERE (SELECT jezikID FROM jezik WHERE imeJezika = '${payload.jezik}' LIMIT 1) IS NULL;`;
}

function ADD_GOVORI(payload) {
    return `INSERT INTO govori (userID, jezikID, citanje, pisanje, razumevanje, govor, sertifikat) SELECT ${payload.userID}, (SELECT jezikID FROM jezik WHERE imeJezika = '${payload.jezik}' LIMIT 1), '${payload.nivoCitanja}', '${payload.nivoPisanja}', '${payload.nivoRazumevanja}', '${payload.nivoGovora}', '${payload.sertifikat}';`;
}

function GET_GOVORI_ID(payload) {
    return `SELECT govoriID FROM govori WHERE userID = ${payload.userID} AND jezikID = (SELECT jezikID FROM jezik WHERE imeJezika = '${payload.jezik}' LIMIT 1);`;
}

function EDIT_GOVORI(payload) {
    return `UPDATE govori SET jezikID = (SELECT jezikID FROM jezik WHERE imeJezika = '${payload.jezik}' LIMIT 1), citanje = '${payload.nivoCitanja}', pisanje = '${payload.nivoPisanja}', razumevanje = '${payload.nivoRazumevanja}', govor = '${payload.nivoGovora}', sertifikat = '${payload.sertifikat}' WHERE govoriID = ${payload.id};`;
}

function DELETE_GOVORI(payload) {
    return `DELETE FROM govori WHERE govoriID = ${payload}`;
}
function ADD_OSTALE_VESTINE(payload) {
    return `INSERT INTO ostaleVestine (userID, vozackeDozvole, vestine, osobine, interesovanja) SELECT ${payload.userID}, '${payload.vozackeDozvole}', '${payload.vestine}', '${payload.osobine}', '${payload.interesovanja}';`;
}

function GET_OSTALE_VESTINE_ID(payload) {
    return `SELECT ostaleVestineID FROM ostaleVestine WHERE userID = '${payload.userID}';`;
}

function EDIT_OSTALE_VESTINE(payload) {
    return `UPDATE ostaleVestine SET vozackeDozvole = '${payload.vozackeDozvole}', vestine = '${payload.vestine}', osobine = '${payload.osobine}', interesovanja = '${payload.interesovanja}' WHERE ostaleVestineID = ${payload.id};`;
}

function DELETE_OSTALE_VESTINE(payload) {
    return `DELETE FROM ostaleVestine WHERE ostaleVestineID = ${payload}`;
}

function ADD_PICTURE (payload, userID) {
    return `UPDATE licniPodaci SET profilnaSlika = '${payload}' WHERE userID = ${userID}`;
}

function ADD_CV (payload, userID) {
    console.log(userID);
    return `UPDATE licniPodaci SET cv = '${payload}' WHERE userID = ${userID}`;
}
